...
package object generic {
  ...
  // -------------------------------------------------------------------------------------------------------------------
  implicit def coproductCellDecoder[H, T <: Coproduct](implicit dh: CellDecoder[H], dt: CellDecoder[T]): CellDecoder[H :+: T] =
  ...
  // -------------------------------------------------------------------------------------------------------------------
  implicit def coproductCellEncoder[H, T <: Coproduct](implicit eh: CellEncoder[H], et: CellEncoder[T]): CellEncoder[H :+: T] =
  ...
  implicit def caseClass1RowDecoder[A, H, R <: HList](implicit gen: Generic.Aux[A, R], ev: (H :: HNil) =:= R, dh: RowDecoder[H]): RowDecoder[A] =
    RowDecoder(s ⇒ dh.decode(s).map(h ⇒ gen.from(ev(h :: HNil))))

  // Case class of arity 2+
  implicit def caseClassNRowDecoder[A, H1, H2, R <: HList](implicit gen: Generic.Aux[A, R], ev: R <:< (H1 :: H2 :: HList), dr: RowDecoder[R]): RowDecoder[A] =
  ...
  implicit def caseClass1RowEncoder[A, H, R <: HList](implicit gen: Generic.Aux[A, R], ev: R <:< (H :: HNil), eh: RowEncoder[H]): RowEncoder[A] =
    RowEncoder(a ⇒ eh.encode(ev(gen.to(a)).head))

  // Case class of arity >= 2
  implicit def caseClassNRowEncoder[A, H1, H2, R <: HList](implicit gen: Generic.Aux[A, R], ev: R <:< (H1 :: H2 :: HList), er: RowEncoder[R]): RowEncoder[A] =
  ...
  implicit def caseObjectCellDecoder[A, R <: HNil](implicit gen: Generic.Aux[A, R], ev: HNil =:= R): CellDecoder[A] =
    CellDecoder(s ⇒ if(s.isEmpty) DecodeResult.success(gen.from(ev(HNil))) else DecodeResult.outOfBounds(0))

  implicit def caseClassCellDecoder[A, R, H](implicit gen: Generic.Aux[A, R], ev: (H :: HNil) =:= R, dh: CellDecoder[H]): CellDecoder[A] =
  ...
  implicit def caseClassCellEncoder[A, R, H](implicit gen: Generic.Aux[A, R], ev: R <:< (H :: HNil), eh: CellEncoder[H]): CellEncoder[A] =
  ...
  // -------------------------------------------------------------------------------------------------------------------
  implicit def coproductRowDecoder[H, T <: Coproduct](implicit dh: RowDecoder[H], dt: RowDecoder[T]): RowDecoder[H :+: T] =
