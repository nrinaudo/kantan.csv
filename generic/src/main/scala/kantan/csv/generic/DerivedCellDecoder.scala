...
object DerivedCellDecoder {
  def apply[A](f: String ⇒ csv.DecodeResult[A]): DerivedCellDecoder[A] = new DerivedCellDecoder[A] {
  ...
  implicit val cnil: DerivedCellDecoder[CNil] = DerivedCellDecoder(_ ⇒ csv.DecodeResult.decodeFailure)
  ...
  implicit def caseObject[A, R <: HNil](implicit gen: Generic.Aux[A, R], ev: HNil =:= R): DerivedCellDecoder[A] =
    DerivedCellDecoder(s ⇒ if(s.isEmpty) csv.DecodeResult.success(gen.from(ev(HNil))) else csv.DecodeResult.decodeFailure)
