...
object DerivedCellDecoder {
  ...
  implicit val cnil: DerivedCellDecoder[CNil] = DerivedCellDecoder(_ ⇒ DecodeResult.outOfBounds(0))
  ...
  implicit def caseObject[A, R <: HNil](implicit gen: Generic.Aux[A, R], ev: HNil =:= R): DerivedCellDecoder[A] =
    DerivedCellDecoder(s ⇒ if(s.isEmpty) DecodeResult.success(gen.from(ev(HNil))) else DecodeResult.outOfBounds(0))
