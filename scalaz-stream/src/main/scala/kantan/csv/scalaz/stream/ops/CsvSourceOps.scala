...
import kantan.csv.scalaz.stream.CsvSource

class CsvSourceOps[A](val a: A) extends AnyVal {
...
trait ToCsvSourceOps {
  implicit def toCsvSourceOps[A](a: A): CsvSourceOps[A] = new CsvSourceOps(a)
}

object csvSource extends ToCsvSourceOps
