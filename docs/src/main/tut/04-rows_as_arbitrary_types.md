...
There are various ways to implement an instance of [`RowDecoder`], but by far the most idiomatic one is to use one of 
...
The main reason this is the preferred solution is that it allows us never to have to think about each cell in a row and
how to decode them - we just have to describe what type we're expecting and let kantan.csv deal with decoding for us.
